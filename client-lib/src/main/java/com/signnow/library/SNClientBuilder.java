...
public class SNClientBuilder {
  ...
  public static SNClientBuilder get() throws SNException {
    ...
    if (localInstance == null) {
      throw new SNException(
          "SNClientBuilder must be initialized with API connection prerequisites") {};
    }
  ...
  private Response getOAuthRequest(Form authForm) throws SNAuthException {
    ...
    if (response.getStatus() >= 500) {
      throw new SNAuthException(
          response.readEntity(ApiError.class).errors.stream()
  ...
  /**
   * Get new API client instance for already authenticated user. Argument object can be obtained
   * from {@link SNClient#getUser()}
   *
   * @param user
   * @return
   * @throws SNAuthException
  ...
  public String createUser(String email, String password) throws SNAuthException {
    try {
      ...
      if (response.getStatus() >= 400) {
        throw new SNAuthException(response.readEntity(ApiError.class).errors.get(0).message);
      } else {
