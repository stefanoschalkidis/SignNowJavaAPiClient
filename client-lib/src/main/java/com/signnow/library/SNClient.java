...
public class SNClient implements ServiceProvider {
  ...
  public static void checkAPIException(Response response) throws SNException {
    if (response.getStatus() == 401 || response.getStatus() == 403) {
      throw new SNApiException(
          response.getStatus() + ": " + response.readEntity(AuthError.class).error);
    } else if (response.getStatus() >= 400) {
      throw new SNApiException(response.readEntity(ApiError.class).errors);
    }
  }
  ...
  public <T> T get(String path, Map<String, String> parameters, Class<T> returnType)
      throws SNException {
  ...
  public <T> T get(String path, Map<String, String> parameters, GenericType<T> returnType)
      throws SNException {
    Response response = buildRequest(path, parameters).get();
    checkAPIException(response);
    return response.readEntity(returnType);
  ...
  public <E, T> T post(
      String path, Map<String, String> parameters, E inputData, Class<T> returnType)
      throws SNException {
    Response response =
        buildRequest(path, parameters)
            .post(Entity.entity(inputData, SNClientBuilder.defaultVariant));
    checkAPIException(response);
    return response.readEntity(returnType);
  }

  public <E, T> T post(
      String path, Map<String, String> parameters, E inputData, GenericType<T> returnType)
      throws SNException {
    Response response =
        buildRequest(path, parameters)
            .post(Entity.entity(inputData, SNClientBuilder.defaultVariant));
  ...
  public <E, T> T put(String path, Map<String, String> parameters, E inputData, Class<T> returnType)
      throws SNException {
    Response response =
        buildRequest(path, parameters)
            .put(Entity.entity(inputData, SNClientBuilder.defaultVariant));
    checkAPIException(response);
    return response.readEntity(returnType);
  }

  public <T> T delete(String path, Map<String, String> parameters, Class<T> returnType)
      throws SNException {
